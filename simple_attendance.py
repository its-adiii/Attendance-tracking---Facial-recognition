...
class SimpleAttendanceSystem:
    def __init__(self, config_file='config.ini'):
        ...
        self.late_time = datetime.now().replace(hour=hour, minute=minute, second=0, microsecond=0).time()
    ...
    def register_new_student(self, name, roll_number, email=None):
        ...
        try:
            ...
            # Add to database (without face encoding for now)
            student_id = self.db_manager.add_student(name, roll_number, email, None)
    ...
    def manual_attendance_mark(self, roll_number, status='present'):
        ...
        # Determine if late
        if status == 'present' and current_time.time() > self.late_time:
            status = 'late'
        
        success = self.db_manager.mark_attendance(student_id, current_time, status)
        
        if success:
            return True, f"Attendance marked for {student[1]} as {status}"
        else:
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            roll_number = input("Enter roll number: ")
            status = input("Enter status (present/late/absent): ") or "present"
